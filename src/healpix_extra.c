...
void he_alm2map(int nside,int lmax,int ntrans,int spin,flouble **maps,fcomplex **alms)
{
  ...
  for(itrans=0;itrans<nbatches;itrans++) {
    sht_wrapper(spin,lmax,nside,HE_MAX_SHT,&(maps[itrans*nmaps*HE_MAX_SHT]),
		&(alms[itrans*nmaps*HE_MAX_SHT]),SHARP_ALM2MAP);
  }
  if(nodd>0) {
    sht_wrapper(spin,lmax,nside,nodd,&(maps[nbatches*nmaps*HE_MAX_SHT]),
		&(alms[nbatches*nmaps*HE_MAX_SHT]),SHARP_ALM2MAP);
  }
}
...
void he_map2alm(int nside,int lmax,int ntrans,int spin,flouble **maps,fcomplex **alms,int niter)
{
  ...
  for(itrans=0;itrans<nbatches;itrans++) {
    sht_wrapper(spin,lmax,nside,HE_MAX_SHT,&(maps[itrans*nmaps*HE_MAX_SHT]),
		&(alms[itrans*nmaps*HE_MAX_SHT]),SHARP_MAP2ALM);
  }
  if(nodd>0) {
    sht_wrapper(spin,lmax,nside,nodd,&(maps[nbatches*nmaps*HE_MAX_SHT]),
		&(alms[nbatches*nmaps*HE_MAX_SHT]),SHARP_MAP2ALM);
  }
  ...
  if(niter) {
    ...
    for(iter=0;iter<niter;iter++) {
      //Get new map
      for(itrans=0;itrans<nbatches;itrans++) {
	sht_wrapper(spin,lmax,nside,HE_MAX_SHT,&(maps_2[itrans*nmaps*HE_MAX_SHT]),
		    &(alms[itrans*nmaps*HE_MAX_SHT]),SHARP_ALM2MAP);
      }
      if(nodd>0) {
	sht_wrapper(spin,lmax,nside,nodd,&(maps_2[nbatches*nmaps*HE_MAX_SHT]),
		    &(alms[nbatches*nmaps*HE_MAX_SHT]),SHARP_ALM2MAP);
      }
	...
	for(ip=0;ip<npix;ip++)
      ...
      for(itrans=0;itrans<nbatches;itrans++) {
	sht_wrapper(spin,lmax,nside,HE_MAX_SHT,&(maps_2[itrans*nmaps*HE_MAX_SHT]),
		    &(alms_2[itrans*nmaps*HE_MAX_SHT]),SHARP_MAP2ALM);
      }
      if(nodd>0) {
	sht_wrapper(spin,lmax,nside,nodd,&(maps_2[nbatches*nmaps*HE_MAX_SHT]),
		    &(alms_2[nbatches*nmaps*HE_MAX_SHT]),SHARP_MAP2ALM);
      }
...
void he_anafast(flouble **maps_1,flouble **maps_2,int pol_1,int pol_2,flouble **cls,int nside,int lmax,int iter)
{
...
{
  long l;
  double sigma=HE_FWHM2SIGMA*fwhm_amin;
			  ...
			  int return_alm,int pol,int input_TEB,int output_TEB)
{
  ...
  for(j=0;j<par->nj;j++) {
    ...
    for(imap=0;imap<nmaps;imap++) {
      ...
      he_zero_alm(l_max,alm_dum[imap]);
    ...
    //SHT^-1
    he_alm2map(par->nside_arr[j],par->lmax_arr[j],0,1,&(nt[j][0]),&(alm_dum[0]));
    if(pol) {
      if(output_TEB)
